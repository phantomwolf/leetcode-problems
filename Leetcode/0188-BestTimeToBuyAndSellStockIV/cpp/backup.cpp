...
public:
    int maxProfit(int k, vector<int> &prices) {
        ...
        while (p < length) {
            ...
            while (!stk.empty() && prices[v] < prices[stk.top().first]) {
                profits.push_back(prices[stk.top().second-1] - prices[stk.top().first]);
                stk.pop();
            }
            ...
            while (!stk.empty() && prices[p-1] >= prices[stk.top().second-1]) {
                profits.push_back(prices[stk.top().second-1] - prices[v]);
                v = stk.top().first;
                stk.pop();
            }
            stk.push(pair<int, int>(v, p));
        }
