...
public:
    int maxProfit(int k, vector<int> &prices)
    {
        ...
        while (i < length) {
            ...
            peak = prices[i - 1];
            // save profit of pairs than can't be merged
            while (!stk.empty() && valley < stk.top().first) {
                profits.push_back(stk.top().second - stk.top().first);
                stk.pop();
            }
            // merge pairs
            while (!stk.empty() && peak >= stk.top().second) {
                profits.push_back(stk.top().second - valley);
                valley = stk.top().first;
                stk.pop();
            }
            stk.push(pair<int, int>(valley, peak));
        }
        // save profits of remaining pairs
        while (!stk.empty()) {
            profits.push_back(stk.top().second - stk.top().first);
            stk.pop();
        }
        // build max heap from profits
        int max_profit = 0;
        priority_queue<int, vector<int>, std::less<int>> pqueue(std::less<int>(), profits);
        for (int i = 0; i < k && !pqueue.empty(); ++i) {
            max_profit += pqueue.top();
            pqueue.pop();
        }
        return max_profit;
    }
